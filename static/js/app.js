...
function buildCharts(gender=null, dept=null) {
  ...
  d3.json('/api/gender_demographic').then(data => {
   ...
   if(gender) {
    ...
    jobsatisfaction = sortedData.map(d => d['jobsatisfaction']);
  ...
  var gaugeData = [
    {
        domain: { x: [0, 1], y: [0, 1] },
        value: 2.72,
        title: { text: "Job Satisfaction by Department" },
        type: "indicator",
        mode: "gauge+number",
        delta: { reference: 400 },
...
Plotly.newPlot('job-satisfaction', gaugeData, layout);
...
//     sortedData = data.sort((d1, d2) => {(d2['jobsatisfaction'].toString() - d1['jobsatisfaction'].toString())});
//     jobsatisfaction = sortedData.map(d => d['jobsatisfaction']);
...
// buildCharts();
// the start of gauge chart


// function buildGage (dept=null, ){

//   d3.json('/api/job_satisfaction_avg').then(function(data) {

//   var happiness = data.map(d => d['job_satisfaction']);
...
//   Plotly.newPlot('job-satisfaction', data, layout);
//   });
